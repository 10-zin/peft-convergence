...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default="constant",
        choices=["constant", "linear", "cosine", "warmup", "plateau"],
...
def get_optimizer(optimizer_name, model, lr):
    if optimizer_name == "adamw":
        ...
        try:
            ...
            return Lion(model.parameters(), lr=lr, weight_decay=args.weight_decay)
        except ImportError:
    ...
    if dataset_name == "ag_news":
        ...
        if model_type == "seq_cls":
            ...
            def tokenize_function(examples):
                ...
                label_max_length = 8 if dataset_name in ["ag_news", "cola"] else 128
                # Tokenize labels
                with tokenizer.as_target_tokenizer():
                    labels_encodings = tokenizer(
                        labels, padding="max_length", truncation=True, max_length=label_max_length
                    )
        ...
        def tokenize_function(examples):
            model_inputs = tokenizer(
                examples["dialogue"], padding="max_length", truncation=True, max_length=512
            )
            with tokenizer.as_target_tokenizer():
                labels_encodings = tokenizer(
        ...
        tokenized_dataset = dataset.map(tokenize_function, batched=True)
        return tokenized_dataset, None  # No classification labels
...
def train_and_evaluate(args):
    ...
    if args.log_wandb:
        ...
        wandb.init(
            project="peft-convergence-samsum",
            config=vars(args),
            name=(
                f"{training_type}_{args.optimizer}_{args.lr_schedule}"
                f"_lr{args.learning_rate}_wd{args.weight_decay}_{args.dataset_name}"
    ...
    if model_type == "seq_cls":
        ...
        train_dataloader = DataLoader(
            ...
            collate_fn=lambda batch: {
                ...
                "decoder_attention_mask": (torch.stack([torch.tensor(x["labels"]) for x in batch]) != tokenizer.pad_token_id).long(),
            },
        )
...
#             optimizer, mode='min', factor=0.5, patience=2, verbose=True
    ...
    for epoch in range(args.num_epochs):
        ...
        for batch in eval_dataloader:
            with torch.no_grad():
                if model_type == "seq_cls":
                    ...
                    generate_max_length = 8 if args.dataset_name in ["ag_news", "cola"] else 128
                    ...
                    if args.dataset_name in ["ag_news", "cola"]:
                        preds = [get_class_from_generated_text(t) for t in generated_texts]
                        labels = [get_class_from_generated_text(t) for t in decoded_labels]
                    else:  # samsum, e2e_nlg
        ...
        # Calculate metrics
        if args.dataset_name in ["samsum", "e2e_nlg"]:
            rouge = load("rouge")
            results = rouge.compute(predictions=all_preds, references=all_labels, use_stemmer=True)
        ...
        if args.lr_schedule == "plateau":
#             lr_scheduler.step(eval_loss / num_eval_batches)
...
#                 convergence_counter = 0
        ...
        if rougeL > best_accuracy:
            best_accuracy = rougeL
            print(f"New best accuracy: {rougeL:.4f}")
    ...
    log_metrics(global_step, final_metrics, args.log_wandb)
